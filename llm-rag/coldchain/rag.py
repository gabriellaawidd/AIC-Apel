...
_KB_COMMODITY = {
    "ikan_segar": "ikan", "ikan": "ikan", "fish": "ikan",
...
def retrieve_advisory(kb: KnowledgeBase, commodity: str, risk_level: str,
    ...
    kb_key = _KB_COMMODITY.get((commodity or "").strip().lower(), commodity)
    query = f"penanganan {kb_key} {risk_terms.get(risk_level, '')}"
    snippets = kb.retrieve(query, commodity=kb_key, top_k=top_k)
    if not snippets:
